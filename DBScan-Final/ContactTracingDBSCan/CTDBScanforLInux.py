...
root.geometry("1440x900") #edit window
...
def start_program():
    ...
    contact_tracing_Button = Button(root, text="Contact Tracing", padx=15, pady=15, font=('Times', 35),
                                    command=clear_start_contact_tracing)
    contact_tracing_Button.place(x=1102, y=729)

    monitoring_button = Button(root, text="Entrance Monitoring", padx=15, pady=15, font=('Times', 35),
                                    command=clear_start_entrance_monitoring)
    monitoring_button.place(x=680, y=729)
...
def contacttracing_output():
    ...
    if len(final_names) > 0:
        for i in final_names:
            my_button = Button(myframe, text=f"Name: {i[0]}     DateTime: {i[1]}     Temp: {i[3]}      \nRoom: {i[2]}      Course: {i[4]}", width=82, font=('Times', 20), command=lambda button_text=i[0]: click_name(button_text)).pack()
...
def change_color_green_rfid():
    # Change the color of the button to green
    scan_button.config(bg='green')

def change_color_green_temp():
    # Change the color of the button to green
    scan_temp_button.config(bg='green')

# def change_color_default():
#     scan_button.config(bg='whtie')
#     scan_temp_button.config(bg='white')
